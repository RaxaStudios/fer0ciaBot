...
public class LotteryHandler {
    ...
    public static class SongList {
        ...
        public synchronized void addUser(String user, String content) {
            ...
            //TODO implement system for duplication check
            // prompt user to re-enter with a new song
            //check that content is a number between 1 and max song #
            try {
                int intContent = Integer.parseInt(content);
                if (intContent < 1 && intContent > 369) {
                    sendMessage("@" + displayName + ", song choice must be a number between 1 and 369");
                    return;
                } else {
            ...
            for (String u : prevWinner) {
                if (u.equals(user)) {
                    if (tempCountFix < 1) {
                        p = false;
                        sendMessage(displayName + " re-added with song: " + content);
                        ticketValue = 1;
                        tempCountFix++;
                    }
                }
        ...
        public synchronized void songClear() {
            ...
            sendMessage("Song lottery has been cleared");
        }
